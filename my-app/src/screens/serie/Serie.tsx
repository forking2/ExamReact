...
import {useSeriesByIdQuery} from "@/screens/serie/hooks/useSeriesByIdQuery.ts";
import {useTranslation} from "react-i18next";
import FavoriteButton from "@/components/favoriteButton/FavoriteButton.tsx";

const Serie = () => {
    const { id } = useParams<{ id: string }>();
    const {data, isLoading} = useSeriesByIdQuery(id!);
    ...
    if (!data) {
        return (
            <div className="flex flex-col items-center justify-center min-h-screen bg-[var(--bg)] text-[var(--text-muted)]">
                ...
                <p className="text-lg font-medium">{t("details.series_not_found")}</p>
            </div>
        );
    }
    ...
    return (
        <div className="min-h-screen bg-[var(--bg)]">
            ...
            <div className="relative w-full h-[40vh] md:h-[50vh] overflow-hidden">
                <img
                    src={SeriesService.getPoster(data.backdrop_path, 'original')}
                    alt={data.name}
            ...
            <div className="max-w-4xl mx-auto px-4 sm:px-6 -mt-16 md:-mt-24 relative z-10 pb-12">
                <div className="flex flex-col md:flex-row gap-6">
                    ...
                    <img
                        src={SeriesService.getPoster(data.poster_path, 'w300')}
                        alt={data.name}
