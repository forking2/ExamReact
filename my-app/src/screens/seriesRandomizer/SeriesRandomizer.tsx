...
import {useSeriesQuery} from "@/screens/seriesRandomizer/hooks/useSeriesQuery.ts";
import {SeriesService} from "@/services/series.service.ts";

const SeriesRandomizer = () => {
    const [randomPage, setRandomPage] = useState(1);
    const [isSpinning, setIsSpinning] = useState(false);
    const [filtersOpen, setFiltersOpen] = useState(false);
    const [picked, setPicked] = useState<null | { id: number; name: string; poster_path: string; vote_average: number }>(null);

    const {filters, setFilters, resetFilters} = useFilmFilterStore();
    const {data, isLoading} = useSeriesQuery(randomPage);
        ...
        <div className="min-h-screen flex items-center justify-center p-4 sm:p-6" style={{ background: 'linear-gradient(135deg, var(--bg), var(--bg-muted))' }}>
            <div className="w-full max-w-sm bg-[var(--bg-card)]/80 backdrop-blur-xl rounded-[2rem] shadow-xl border border-[var(--border)] px-4 sm:px-7 pt-6 pb-7 flex flex-col items-center gap-1">
                ...
                {picked && !isSpinning && (
                    ...
                    >
                        <img
                            src={SeriesService.getPoster(picked.poster_path, 'w200')}
                            alt={picked.name}
                        ...
                        <div className="flex flex-col justify-center gap-1 min-w-0">
                            ...
                            <span className="text-violet-500 text-xs font-medium mt-1">{t('series_randomizer.view_detail')} →</span>
                        </div>
                    </Link>
                )}
                ...
                >
                    ...
                    {isSpinning ? t("series_randomizer.button_rolling") : t("series_randomizer.button")}
                </button>
...
export default SeriesRandomizer;
